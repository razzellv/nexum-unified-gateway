...
const Settings = () => {
  ...
  // Budget state (executive/admin only)
  const [budgetData, setBudgetData] = useState<any>(null);
  const [budgetLoading, setBudgetLoading] = useState(false);
  const [deptBudgets, setDeptBudgets] = useState<any[]>([]);

  // Utilities state (executive/admin only)
  ...
  useEffect(() => {
    if (activeTab === 'budget' || activeTab === 'utilities') {
      fetchBudget();
    }
    if (activeTab === 'team') {
      fetchTeam();
    ...
    try {
      await fetch(`${baseUrl}/onboarding/utilities`, {
        ...
        body: JSON.stringify({
          ...
          budget: {
            ...(budgetData || {}),
            departments: deptBudgets.filter(d => d.department),
          },
        }),
      });
      toast({ title: 'Budget saved', description: 'Budget configuration updated.' });
    } catch (err) {
      ...
      <div className="space-y-4 md:space-y-6">
        ...
        <div className="flex flex-col md:flex-row gap-4 md:gap-6">
          ...
          <div className="flex-1 glass-panel p-4 md:p-6 min-w-0">
            ...
            {activeTab === 'profile' && (
              <div className="space-y-6">
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  ...
                  <div className="space-y-2">
                    <Label>Phone</Label>
                    <Input value={profile.phone} onChange={e => setProfile({ ...profile, phone: e.target.value })} placeholder="(555) 000-0000" />
                  </div>
                </div>
                <Button onClick={() => toast({ title: 'Profile saved' })}>
                  ...
                  <Card className="border-border/50">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Managed through Cognito — use forgot password to reset</p>
                      </div>
                  ...
                  <Card className="border-border/50">
                    <CardContent className="p-4 flex items-center justify-between">
                      <div>
                        ...
                        <p className="text-xs text-muted-foreground">Tokens expire automatically — re-login required after 1 hour</p>
                      </div>
              ...
              <div className="space-y-6">
                ...
                {!can(userRole, ADMIN_ROLES) && (
                  <div className="flex items-center gap-2 p-3 rounded-lg bg-muted/30 text-sm text-muted-foreground">
                    ...
                    View only — contact your admin to make team changes
                  </div>
                )}
                {teamLoading ? (
                  ...
                  <div className="text-center py-8 text-muted-foreground text-sm">
                    No team members found. Staff can be added through the onboarding wizard.
                  </div>
                ) : (
                  <div className="space-y-2">
            ...
            {/* ── Budget (executive/admin only — editable) ── */}
            {activeTab === 'budget' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                      ...
                      <CardContent className="space-y-4">
                        <div className="grid grid-cols-2 gap-4">
                          <div className="col-span-2 space-y-2">
                            ...
                            <Input
                              type="number"
                              value={budgetData?.annualTotal || ''}
                              onChange={e => setBudgetData({ ...(budgetData || {}), annualTotal: e.target.value })}
                              placeholder="e.g., 500000"
                            />
                            {budgetData?.annualTotal && (
                              <p className="text-xs text-muted-foreground">
                                Monthly: ${(parseFloat(budgetData.annualTotal) / 12).toLocaleString('en-US', { maximumFractionDigits: 0 })} / mo
                              </p>
                            )}
                          </div>
                          <div className="space-y-2">
                        ...
                        {deptBudgets.map((dept, i) => (
                          <div key={i} className="grid grid-cols-5 gap-2 items-end">
                            <div className="col-span-2 space-y-1">
                              ...
                              <Input value={dept.department} onChange={e => updateDept(i, 'department', e.target.value)} placeholder="e.g., Maintenance" />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-xs">Annual ($)</Label>
                              <Input type="number" value={dept.annualBudget} onChange={e => updateDept(i, 'annualBudget', e.target.value)} placeholder="0" />
                            </div>
                            <div className="space-y-1">
                              <Label className="text-xs">Monthly ($)</Label>
                              <Input type="number" value={dept.monthlyBudget} onChange={e => updateDept(i, 'monthlyBudget', e.target.value)} placeholder="Auto" />
                            </div>
                            <Button variant="ghost" size="icon" onClick={() => setDeptBudgets(deptBudgets.filter((_, idx) => idx !== i))} className="text-destructive hover:text-destructive">
            ...
            {/* ── Utility Rates (executive/admin only) ── */}
            {activeTab === 'utilities' && (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={int.name} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                      ...
                      <Badge className={int.status === 'connected' ? 'bg-green-500/20 text-green-400' : 'bg-yellow-500/20 text-yellow-400'}>
                        {int.status}
                      </Badge>
                    </div>
                  ))}
                ...
                <div className="space-y-3">
                  {[
                    ...
                    <div key={item.label} className="flex items-center justify-between p-3 rounded-lg bg-muted/30">
                      ...
                      <Badge className={item.status === 'healthy' ? 'bg-green-500/20 text-green-400' : 'bg-blue-500/20 text-blue-400'}>
                        {item.status}
                      </Badge>
                    </div>
                  ))}
