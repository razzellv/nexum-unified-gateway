...
const systemConfig: Record<SystemType, { icon: typeof Flame; label: string; color: string; bgColor: string }> = {
  boiler: { icon: Flame, label: 'Boiler', color: 'text-boiler', bgColor: 'bg-boiler-muted border-boiler/40' },
  chiller: { icon: Snowflake, label: 'Chiller', color: 'text-chiller', bgColor: 'bg-chiller-muted border-chiller/40' },
  pump: { icon: Gauge, label: 'Pump / Compressor', color: 'text-pump', bgColor: 'bg-pump-muted border-pump/40' },
  ahu: { icon: Wind, label: 'AHU / RTU', color: 'text-ahu', bgColor: 'bg-ahu-muted border-ahu/40' },
  tower: { icon: Droplets, label: 'Cooling Tower', color: 'text-tower', bgColor: 'bg-tower-muted border-tower/40' },
  energy: { icon: Zap, label: 'Energy & Utilities', color: 'text-energy', bgColor: 'bg-energy-muted border-energy/40' },
};
...
export function SystemSelector({ onSelect }: SystemSelectorProps) {
  ...
  const handleEnergyLogSelect = () => {
    if (facility && building) {
      onSelect(facility, building, null); // null system for facility-level energy log
    }
  };
      ...
      <div className="w-full max-w-2xl space-y-8">
        ...
        <div className="glass-panel rounded-xl p-6 space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="input-group">
              ...
              <Select value={selectedFacility} onValueChange={(v) => { setSelectedFacility(v); setSelectedBuilding(''); }}>
        ...
        {building && groupedSystems && (
          <div className="space-y-4 animate-fade-in">
            ...
            {/* Energy & Utilities - Building Level (no equipment required) */}
            <div className="space-y-2">
              <div className="flex items-center gap-2 px-1">
                ...
                <span className="text-sm font-medium text-muted-foreground">{systemConfig.energy.label}</span>
              </div>
              <button
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              {(Object.entries(groupedSystems) as [SystemType, SystemInfo[]][]).map(([type, systems]) => {
                      ...
                      >
                        <div className="flex items-center justify-between">
                          <div>
                            ...
                            <p className="text-sm text-muted-foreground font-mono">{system.assetTag}</p>
                          </div>
